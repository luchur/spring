...
public class DefaultBeanDefinitionDocumentReader implements BeanDefinitionDocumentReader {
	 ...
	 * 如果是默认的命名空间，则使用parseDefaultElement方法进行元素解析，
	 * 否则使用parseCustomElement方法进行解析
	 */
	...
	protected void importBeanDefinitionResource(Element ele) {
		...
		if (absoluteLocation) {
			...
			try {
				int importCount;
				//Resource存在多个子实现类,每个createRelative的实现方式都不一样,所以这里先使用子类的方法尝试解析
